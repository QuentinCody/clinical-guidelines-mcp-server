...
export function registerGetSchema(server: McpServer, env?: SchemaEnv) {
	const handler = createGetSchemaHandler("GUIDELINES_DATA_DO", "guidelines");

	server.registerTool(
		"guidelines_get_schema",
		{
			title: "Get Staged Data Schema",
			description:
				"Get schema information for staged clinical guidelines data. Shows table structures and row counts. " +
